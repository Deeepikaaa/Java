...
//	result = webmethod.addNumbers(a, b);
	...
	public void getNumbers(int int1, int int2) throws IOException {
		...
		StringEntity xmlEntity = new StringEntity(
				"<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ser=\"http://service.soap.bdd.com\"> <soapenv:Header/> <soapenv:Body> <ser:addNumbers> <ser:num1>"+int1+"</ser:num1> <ser:num2>"+int2+"</ser:num2> </ser:addNumbers> </soapenv:Body> </soapenv:Envelope>");
		System.out.println(xmlEntity);
		HttpPost post = new HttpPost("http://localhost:9090/soap-bdd-demo/services/AddTwoNumbers");
		...
		try {
			...
			Document doc = dBuilder.parse(new InputSource(new ByteArrayInputStream(responseXml.getBytes("UTF-8"))));
